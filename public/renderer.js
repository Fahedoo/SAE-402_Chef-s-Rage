...
const GET_LEVEL_CONFIGS = (canvasWidth) => ({
    ...
    ennemi: {
        platforms: [
            ...
            { x: 92,  y: 275, w: canvasWidth - 133, h: 18, slope: -65  }, 
...
export class GameRenderer {
    ...
    initImages(color) {
        ...
        const toLoad = {
            ...
            climb: `assets/rat_echelle_${color}.png`,
    ...
    drawLadders(laddersArray, isBroken = false) {
        this.ctx.save();
        laddersArray.forEach(lad => {
            ...
            if (isBroken) {
                ...
                drawSegment(centerTopY, centerBotY);
            }
        });
        this.ctx.restore();
    }

    draw(state = { players: {}, tomatoes: [], hearts: [], knives: [], levers: [], baskets: [], rollers: [] }) {
        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
        
        if (state.cheeseActive !== undefined) {
            this.cheeseActive = state.cheeseActive;
        }
        ...
        // 4. PROJECTILES, BONUS, PANIERS ET ROULEAUX
        if (state.tomatoes) state.tomatoes.forEach(t => {
            if (this.images.tomate.complete && this.images.tomate.naturalWidth > 0) {
                this.ctx.drawImage(this.images.tomate, t.x, t.y, 30, 30);
            }
        });
        
        if (state.knives) state.knives.forEach(k => {
            if (this.images.knife.complete && this.images.knife.naturalWidth > 0) {
                this.ctx.save();
        ...
        if (state.hearts) state.hearts.forEach(h => {
            if (this.images.coeur.complete && this.images.coeur.naturalWidth > 0) {
                const floatY = Math.sin(Date.now() / 200) * 5; 
                this.ctx.drawImage(this.images.coeur, h.x, h.y + floatY, 30, 30);
            }
        });

        if (state.baskets) state.baskets.forEach(b => {
            if (this.images.panier && this.images.panier.complete && this.images.panier.naturalWidth > 0) {
                const floatY = Math.sin(Date.now() / 150) * 3; 
                this.ctx.drawImage(this.images.panier, b.x, b.y + floatY, 40, 40);
            }
        });

        if (state.rollers) state.rollers.forEach(r => {
            if (this.images.rouleau && this.images.rouleau.complete && this.images.rouleau.naturalWidth > 0) {
                this.ctx.save();
                this.ctx.translate(r.x + r.w / 2, r.y + r.h / 2);
                this.ctx.rotate(r.angle);
                this.ctx.drawImage(this.images.rouleau, -r.w / 2, -r.h / 2, r.w, r.h);
                this.ctx.restore();
            }
        });
        ...
        if (this.mode === 'coop' && state.levers) {
            state.levers.forEach(lev => {
                ...
                if (img.complete && img.naturalWidth > 0) {
                    this.ctx.drawImage(img, lev.x - 20, lev.y - 40, 40, 40);
                }
            });
        }
        ...
        Object.values(state.players || {}).forEach(p => {
            ...
            if (skin && skin.complete && skin.naturalWidth > 0) {
                ...
                // On dessine l'image en l'alignant au bas de la hitbox
                this.ctx.drawImage(skin, -ow / 2, 15 - oh, ow, oh);

                // Overlays Power-ups
                if (p.hasBoost) { this.ctx.fillStyle = "rgba(0,255,0,0.4)"; this.ctx.fillRect(-15, -15, 30, 30); }
                if (p.isTrapped) { this.ctx.fillStyle = "rgba(0,0,0,0.6)"; this.ctx.fillRect(-15, -15, 30, 30); }
                if (p.isInverted) { this.ctx.fillStyle = "rgba(128,0,128,0.5)"; this.ctx.fillRect(-15, -15, 30, 30); }
